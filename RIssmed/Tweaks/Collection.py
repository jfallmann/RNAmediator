...
def parseargs_collectpl():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Cutoff for the definition of pairedness, if set to e.g. 0.2 it will mark all regions with probability of being unpaired >= cutoff as unpaired",
    )
    parser.add_argument(
        "-b",
        "--border",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of parallel processes to run this job with, only important of no border is given and we need to fold",
    )
    parser.add_argument(
        "--loglevel",
        type=str,
...
def parseargs_foldcons():
    ...
    parser.add_argument(
        ...
        type=str,
        default="STDOUT",
        help='Print output of unconstraint folding to file with this name, use "STDOUT" to print to STDOUT (default)',
    )  # or "add" to append to paired/unpaired output')
    parser.add_argument(
        "-n",
        "--unpaired",
        type=str,
...
def parseargs_collectWindow():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Cutoff for the minimum change between unconstraint and constraint structure, regions below this cutoff will not be returned as list of regions with most impact on structure. If not defined, will be calculated from folding the sequence of interest at temperature range 30-44.",
    )
    parser.add_argument("-o", "--outdir", type=str, default="", help="Directory to write to")
    parser.add_argument(
        "-g",

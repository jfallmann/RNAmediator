...
def constrain_seq(
    ...
    if save:
        # 		printlog('SAVING')
        write_constraint(
            fa, paired, unpaired, data_u, data_p, cons, region, diff_nu, diff_np, str(window), outdir
        )
...
def write_unconstraint(fa, unconstraint, data, region, window, outdir):
    ...
    try:
        if unconstraint != 'STDOUT':
            ...
            os.chdir(outdir)
            if not os.path.exists(
                str(goi + '_' + chrom + '_' + strand + '_' + unconstraint + '_' + window + '.gz')
            ):
                o = gzip.open(
                    goi + '_' + chrom + '_' + strand + '_' + unconstraint + '_' + window + '.gz', 'wb'
                )
...
def write_constraint(
    fa, paired, unpaired, data_u, data_p, constrain, region, diff_nu, diff_np, window, outdir
):
    # 	pp = pprint.PrettyPrinter(indent=4)#use with pp.pprint(datastructure)
...
def write_temp(fa, temp, data, region, diff, window, outdir):
    ...
    try:
        ...
        if not os.path.exists(
            ...
            o = gzip.open(
                'TempCons_' + goi + '_' + chrom + '_' + strand + '_' + temp + '_temp_' + window + '.gz', 'wb'
            )
            o.write(bytes(print_up(data['up'], len(fa.seq), region), encoding='UTF-8'))
            o.close()
            if not os.path.exists(

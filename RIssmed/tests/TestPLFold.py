# TestPLFold.py ---
#
# Filename: TestPLFold.py
# Description:
# Author: Joerg Fallmann
# Maintainer:
# Created: Fri Jun 15 14:29:10 2018 (+0200)
# Version:
# Package-Requires: ()
# Last-Updated: Tue Feb  5 17:16:08 2019 (+0100)
#           By: Joerg Fallmann
#     Update #: 57
# URL:
# Doc URL:
...
#load own modules
import sys
sys.path=[str(os.getenv('HOME')+"/Scripts/Python/lib")] + sys.path
...
def fold(sequence, window, span, region, unconstraint, unpaired, paired, length, gc, number, constrain, conslength, alphabet, plot, save, procs, vrna, temprange, outdir, verbosity=False, pattern=None, cutoff=None):
#set path for output
	if outdir:
		printlog(outdir)
...
#print plfold output
					if save:
						write_unconstraint(fa, unconstraint, data, int(region), str(window), outdir)
				else:
#The hard work of folding this has already been done, so we just read in the results
					printlog('Found '+str(goi+'_'+unconstraint+'_'+str(window)+'.gz')+', will read in data and not recalculate')
...
# Create process pool with processes
				...
				if len(fa.seq) > window*4:# If the sequence is very large and we only need a proxy of changes to get a border for the cutoff of CalcConsDiffs we randomly select 10 regions of size window of the sequence for folding
					selection = ''
...
#convert to array for fast diff	calc
				...
				if an[0] == None or len(an) > len(fa.seq): #This means that the prob info was loaded from file or the raw sequence has not been folded yet, but we need a subsequence for the cutoff calculation, so we fold the subsequence
...
##### Functions #####
def constrain_seq(fa, start, end, conslength, const, cons, window, span, region, an, animations, xs, paired, unpaired, save, outdir, plot, data):
#   DEBUGGING
#	pp = pprint.PrettyPrinter(indent=4)#use with pp.pprint(datastructure)
	...
	if os.path.exists('StruCons_'+goi+'_'+chrom+'_'+strand+'_'+cons+'_'+paired+'_'+str(window)+'.gz') and os.path.exists('StruCons_'+goi+'_'+chrom+'_'+strand+'_'+cons+'_'+unpaired+'_'+str(window)+'.gz'):
		return
...
#		printlog('PLOTTING '+str(plot))
	...
	if save:
#		printlog('SAVING')
		write_constraint(fa, paired, unpaired, data_u, data_p, cons, region, diff_nu, diff_np, str(window), outdir)

def constrain_seq_paired(fa, fstart, fend, start, end, conslength, const, cons, window, span, region, an, animations, xs, paired, unpaired, save, outdir, plot, data):
#   DEBUGGING
#	pp = pprint.PrettyPrinter(indent=4)#use with pp.pprint(datastructure)
...
#	pp.pprint(data['up'][1:10])
	...
	try:
		if unconstraint != 'STDOUT':
			...
			os.chdir(outdir)
			if not os.path.exists(str(goi+'_'+chrom+'_'+strand+'_'+unconstraint+'_'+window+'.gz')):
				o = gzip.open(goi+'_'+chrom+'_'+strand+'_'+unconstraint+'_'+window+'.gz', 'wb')
...
def write_constraint(fa, paired, unpaired, data_u, data_p, constrain, region, diff_nu, diff_np, window, outdir):
#	pp = pprint.PrettyPrinter(indent=4)#use with pp.pprint(datastructure)
...
#print outputs to file or STDERR
	try:
		if paired != 'STDOUT':
			bakdir=os.path.abspath(os.getcwd())
			os.chdir(outdir)
			if not os.path.exists('StruCons_'+goi+'_'+chrom+'_'+strand+'_'+constrain+'_'+paired+'_'+window+'.gz'):
				o = gzip.open('StruCons_'+goi+'_'+chrom+'_'+strand+'_'+constrain+'_'+paired+'_'+window+'.gz', 'wb')
		...
		if unpaired != 'STDOUT':
			bakdir=os.path.abspath(os.getcwd())
			os.chdir(outdir)
			if not os.path.exists('StruCons_'+goi+'_'+chrom+'_'+strand+'_'+constrain+'_'+unpaired+'_'+window+'.gz'):
				o = gzip.open('StruCons_'+goi+'_'+chrom+'_'+strand+'_'+constrain+'_'+unpaired+'_'+window+'.gz', 'wb')
		...
		if diff_nu.any():
			if unpaired != 'STDOUT':
				bakdir=os.path.abspath(os.getcwd())
				os.chdir(outdir)
				if not os.path.exists('StruCons_'+goi+'_'+chrom+'_'+strand+'_'+constrain+'_diffnu_'+window+'.gz'):
					o = gzip.open('StruCons_'+goi+'_'+chrom+'_'+strand+'_'+constrain+'_diffnu_'+window+'.gz', 'wb')
					npprint(diff_nu,o)
				os.chdir(bakdir)
			else:
				npprint(diff_nu)
		...
		if diff_np.any():
			if unpaired != 'STDOUT':
				bakdir=os.path.abspath(os.getcwd())
				os.chdir(outdir)
				if not os.path.exists('StruCons_'+goi+'_'+chrom+'_'+strand+'_'+constrain+'_diffnp_'+window+'.gz'):
					o = gzip.open('StruCons_'+goi+'_'+chrom+'_'+strand+'_'+constrain+'_diffnp_'+window+'.gz', 'wb')
					npprint(diff_np,o)
				os.chdir(bakdir)
			else:
				npprint(diff_np)
...
#print outputs to file or STDERR
	...
	try:
		...
		if not os.path.exists('TempCons_'+goi+'_'+chrom+'_'+strand+'_'+temp+'_temp_'+window+'.gz'):
			o = gzip.open('TempCons_'+goi+'_'+chrom+'_'+strand+'_'+temp+'_temp_'+window+'.gz', 'wb')
			o.write(bytes(print_up(data['up'],len(fa.seq),region),encoding='UTF-8'))
			o.close()
			if not os.path.exists('TempCons_'+goi+'_'+chrom+'_'+strand+'_'+temp+'_difft_'+window+'.gz'):
				o = gzip.open('TempCons_'+goi+'_'+chrom+'_'+strand+'_'+temp+'_difft_'+window+'.gz', 'wb')
				npprint(diff,o)
				o.close()
		os.chdir(bakdir)
		return 1
...
if __name__ == '__main__':
	args=parseargs()
	fold(args.sequence, args.window, args.span, args.region, args.unconstraint, args.unpaired, args.paired, args.length, args.gc, args.number, args.constrain, args.conslength, args.alphabet, args.plot, args.save, args.procs, args.vrna, args.temprange, args.outdir, args.verbosity, args.pattern, args.cutoff)

...
** Window folding
*** Preprocess
perl -lane 'if ($F[0]=~ /chr/ && $F[7] =~ /chr/){print join("\t",$F[0],$F[1]-$F[15]+1,$F[2]-$F[15]+1,@F[3..7],$F[8]-$F[15]+1,$F[9]-$F[15]+1,@F[10..$#F])}' Annotated_65_Fu_closest_gene_overlap.bed |perl -lane 'if($F[1]>=0 && $F[2] >= 0 && $F[8] >= 0 && $F[9] >= 0){print $_}' |grep ENSG |sort -k1,1 -k2,2n -u > Local_Paired_65_Fu_constraints.bed
*** Run
python ~/Projects/INPROGRESS/RIssmed/RIssmed/ConstraintFold.py -s 65_Fu_targets.fa.gz -w 250 -l 150 -n unpaired -p paired -r add -x Minimal_paired_65_Fu_constraints.bed -i TargetGenes_65_Fu.bed -z 8 -o Window_paired_65_FU

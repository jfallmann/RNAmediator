...
***** CLI
for i in */Collection_unpaired.bed.gz;do a=${i%\/*}; zcat $i |perl -wlane '{$cons=(split(/\|/,$F[3]))[1];$reg{$cons}{"RAW"}=$F[$#F];$reg{$cons}{$F[6]}=join("|",$F[7],$F[4],$F[-2])}END{foreach $c (keys %reg){$r=$reg{$c}{"RAW"};delete $reg{$c}{"RAW"}; foreach $d (sort{$a <=> $b} keys %{$reg{$c}}){print join("\t",$c,$d,$r,$reg{$c}{$d})}}}' |gzip > Distance_Zscore_$a\.gz;done
***** Slurm
...
OUT=${FILE%\/*}; 
zcat $FILE |perl -wlane '{$cons=(split(/\|/,$F[3]))[1];$reg{$cons}{"RAW"}=$F[$#F];$reg{$cons}{$F[6]}=join("|",$F[7],$F[4],$F[-2])}END{foreach $c (keys %reg){$r=$reg{$c}{"RAW"};delete $reg{$c}{"RAW"}; foreach $d (sort{$a <=> $b} keys %{$reg{$c}}){print join("\t",$c,$d,$r,$reg{$c}{$d})}}}' |gzip > /scratch2/CONSTRAINTS/Distance_Zscore_$OUT\.gz
...
u=${FILE#Distance_Zscore_*};u=${u%.gz}; Rscript --verbose /scratch/CONSTRAINTS/Longwindow/Stats.R $a "GC_"$u"_s250" 10 

...
perl -wlane 'next unless ($F[1]);next if ($F[0]=~/NT_|Best|NC_/ || $F[1] =~ /Best/);print join("\t",$F[1],($F[4]-1),$F[5],$F[0],0,$F[7])' targetmatches |gzip > targetgenes.bed.gz

ca bedtools
zcat GCF_000001405.38_GRCh38.p12_genomic.fna.gz > hg38_refseq109.fa
fastaFromBed -fi hg38_refseq109.fa -bed targetgenes.bed.gz -s -name+ -fullHeader |gzip > targetgenes.fa.gz

perlswitch perl-5.22.2
perl ~/Scripts/Perlen/JoinFiles.pl -f hsa_miRWalk_3UTR.txt,targetgenes.bed.gz -d $'\t' -j 3,4 |gzip > targetinfo

perl -wlane '@c=split(","$F[3]); print join("\t",$F[6],$c[0]-1,$c[1],$F[2]."|".$F[0],)' targetinfo

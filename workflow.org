...
***** Intersect
 intersectBed -s -a Sig65_Collection_unpaired.bed.gz -b Fu_peaks_for_folding.bed -wa -wb |sort -k1,1 -k2,2n|uniq|cat intersect_header -|cut -d$'\t' -f 1-15|sort -k1,1 -k2,2n|uniq|cat intersect_header -|cut -d$'\t' -f 1-15|gzip > Intersect_sig65_Fu.bed.gz
...
*** targetscan
 ...
 perl -wlane '@n=split(":",$F[3]);print join("\t",($F[0]=~s/chr//),$F[1],$F[2],$n[0],$F[4],$F[5],$n[1])' Targetscan_hg19_mir_genomic.bed|sort -k1,1 -k2,2n|gzip > targetsites_targetscan_hg19.bed.gz
 ...
 cut -f4 Targetscan_hg38_mir_genomic.bed|cut -d$':' -f1|sort -u > Targetscan_targetgenes
 rm -f targetmatches_tscan;for i in `cat Targetscan_targetgenes`;do a=$(fgrep "Name=$i;" <(zcat genes.gff3.gz));echo "$i\t$a" >> targetmatches_tscan;done
 perl -wlane 'next unless ($F[1]);next if ($F[0]=~/NT_|Best|NC_/ || $F[1] =~ /Best/);print join("\t","chr".$F[1],($F[4]-1),$F[5],$F[0],0,$F[7])' targetmatches_tscan |sort -k1,1 -k2,2n|grep -v Gnomon|gzip > targetgenes_tscan.bed.gz

 ca bedtools
 #zcat GCF_000001405.38_GRCh38.p12_genomic.fna.gz |perl -wlane 'if ($_ =~ /^>/){@l=split(" ",$_);($h=$l[0])=~s/\>/\>chr/;print $h}else{print}'> hg38_refseq109.fa
 fastaFromBed -fi hg38_refseq109.fa -bed <(zcat targetgenes_tscan.bed.gz|grep -v Gnomon) -s -name+ -fullHeader |gzip > tscan_targetgenes.fa.gz

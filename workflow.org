...
for i in GC*.fa.gz;do zcat $i |perl -wane 'if ($F[0]=~/^>/){($id=(split(":",$F[0]))[0])=~s/>//;$id=~s/_/-/}else{print join("\t","chr1",0,length($F[0]),$id,100,"+")."\n"}' |gzip > $i\.bed.gz;done
for a in 20 40 60 80;do for i in 5 7 9 11;do ~/Projects/INPROGRESS/RIssmed/RIssmed/ConstraintPLFold.py -s GC$a\.fa.gz -w 120 -l 80 -u $i -r raw -n unpaired -p paired -x sliding -o Random_$a\_$i -z 12 --loglevel DEBUG -m 2;done
for a in 20 40 60 80;do for i in 5 7 9 11;do ~/Projects/INPROGRESS/RIssmed/RIssmed/CollectConsResults.py -u $i -z 4 -b-1,1 -c 0 -g GC$a\.fa.gz.bed.gz -p 120,80 -o Collection_$a\_$i -d Random_$a\_$i;done;done
**** SLURM
...
**** DenbiCloud
for i in 20 40 60 80;do python RIssmed/Randseq.py -l 1000 -g $i -n 1000 -o GC$i;done
for i in GC*.fa.gz;do zcat $i|sed 's/_/-/' |gzip > Fixed_$i;done
rm -f genes.bed; for i in Fixed_GC*.fa.gz;do zcat $i|perl -wlane 'if ($_ =~ /^>/){($name = (split(":",$_))[0])=~s/>//;}else{print join("\t","nochrom",0,(length($_)-1),$name,100,".")}' >> genes.bed;done
for a in 20 40 60 80;do for i in 5 7 9 11;do if [ ! -d "Random_$a_$i" ];then sbatch --export ALL,gc=$a,u=$i rissmed_random.sh;fi;done;done
for a in 20 40 60 80;do for i in 5 7 9 11;do sbatch --export ALL,gc=$a,u=$i rissmed_collect.sh ;done;done
*** Statistics
**** Collect subsets
for a in 20 40 60 80;do for i in 5 7 9 11;do sbatch --export ALL,gc=$a,u=$i rissmed_stats.sh;done;done
**** Correlation distance to delta-acc
rm -f Dist_acc.tsv.gz; for a in 20 40 60 80;do for i in 5 7 9 11;do zcat $a\_$i\/Collection_unpaired.bed.gz|cut -f5,7,8,12|sort|perl -wslne 'chomp($line=$_);print $line."\t".$a."_".$i' -- -a=$a -i=$i |gzip >> InfoSubset.tsv.gz;done;done
Accessibility_difference, Distance_to_constraint, Accessibility_no_constraint, Zscore
**** Plot
Stats.R
**** Plot more
ssh circ
cd /scratch/CONSTRAINTS/Longwindow
***** CLI
for i in */Collection_unpaired.bed.gz;do a=${i%\/*}; zcat $i |perl -wlane '{$cons=(split(/\|/,$F[3]))[1];$reg{$cons}{"RAW"}=$F[$#F];$reg{$cons}{$F[6]}=join("|",$F[7],$F[4],$F[-1])}END{foreach $c (keys %reg){print join("\t",$c,$reg{$c}{"RAW"});delete $reg{$c}{"RAW"}; foreach $d (sort{$a <=> $b} keys %{$reg{$c}}){print join("\t",$d,$reg{$c}{$d})}}}' |gzip > Distance_Zscore_$a\.gz;done
***** Slurm
for i in */Collection_unpaired.bed.gz;do sbatch --export ALL,FILE=$i extract_info.sh;done
cd /scratch2/CONSTRAINTS
for i in Distance_Zscore_*.gz;do sbatch --export ALL,FILE=$i plot.sh;done
****** extract_info.sh
#!/bin/bash
#
#SBATCH --job-name=extract
#SBATCH --output=/scratch2/SLURMLOG/extract_%J.txt
#SBATCH --ntasks=10
...
OUT=${FILE%\/*}; 
zcat $FILE |perl -wlane '{$cons=(split(/\|/,$F[3]))[1];$reg{$cons}{"RAW"}=$F[$#F];$reg{$cons}{$F[6]}=join("|",$F[7],$F[4],$F[-1])}END{foreach $c (keys %reg){print join("\t",$c,"0",$reg{$c}{"RAW"});delete $reg{$c}{"RAW"}; foreach $d (sort{
$a <=> $b} keys %{$reg{$c}}){print join("\t",$c,$d,$reg{$c}{$d})}}}' |gzip > /scratch2/CONSTRAINTS/Distance_Zscore_$OUT\.gz
###PreAcc, deltaAcc, zScore

****** plot.sh
!/bin/bash
#
#SBATCH --job-name=plot
#SBATCH --output=/scratch2/SLURMLOG/plot_%J.txt
#SBATCH --ntasks=10
...
u=${FILE#Distance_Zscore_*};u=${u%.gz}; Rscript --verbose Stats.R $a "GC_"$u"_s120" 10


* TODOS
** Soft-constraints
Add factor variable and model variable to define soft-constraints factor specific and binding motif specific
e.g. 'TTP,-0.02,WTTTW;HuR,+0.2,UUUUU' as command-line or file, then --factors TTP,HuR,... and foreach factor in factors run folding and apply soft constraint if motif regex fits or everywhere, needs smart regex to work with IUPAC, see AREsite motif regex builder
